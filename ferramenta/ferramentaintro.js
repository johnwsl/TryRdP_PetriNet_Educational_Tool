//OBSERVAÇÃO: esse arquivo é um arquivo modifidado de ferramenta.js, onde se retirou elementos e propriedades dos elementos das redes de Petri. 
//Transição imediata, arco inibidor e métrica foram retirados. Retirou-se as propriedades das transições 'brancas' deixando apenas o label. 
//Ele é usado apenas na seção 1.
...
function main(bodygraph, qxml, nextpage, ex, pontos)
{
    ...
    //Observação: os atributos de qxml seguem um formato criado pelo programador, explicado no decorrer do código
        ...
        else
        {
            ...
            document.getElementById("enviar").addEventListener("click", function(){gerador(graph, qxml, nextpage, ex, pontos);});
    ...
    function comparador(textxml, MetricaArr, TransImArr, qxml, numLugar, numTransIm, numTransTemp, numMetrica, numArcoInibidor, numArco)
    {
        ...
        if((numLugar==0)&&(MetricaArr==0)&&(TransImArr==0)&&(numTransIm==0)&&(numTransTemp==0)&&(numMetrica==0)&&(numArcoInibidor==0)&&(numArco==0))
        {
            mxUtils.alert("É preciso resolver o exercício para avançar!")
            return null;
        }
    ...
    function selectionChanged(graph, docxml)
    {
        ...
        if (cell != null)
        {
            ...
            else if(mxUtils.isNode(cell.value) && cell.value.nodeName=='TransTemp')// Para o arquivo relacionado a introdução, considera-se apenas o primeiro elemento
            {
                for (var i = 0; i < 1; i++)
                {
                    araux[i]=createTextField(graph, form, cell, attrs[i]);		
            ...
            function() //Função para o botão OK
            {
                ...
                while(auxbool==false)
                {
                    ...
                    else if(mxUtils.isNode(cell.value) && cell.value.nodeName=='TransTemp')// Para o arquivo relacionado a introdução, considera-se o primeiro elemento
                    {                                                                      // da transição temporizada
                        (araux[0])();
                    }
                    else	
                    {	
