...
//A métrica foi retirada. As propriedades da transição 'preta' foram retiradas deixando apenas o label. 
//Ele é usado apenas na seção 2.
...
function main(bodygraph, qxml, nextpage, ex, pontos)
{
    ...
    function selectionChanged(graph, docxml)
    {
        ...
        if (cell != null)
        {
            ...
            else if(mxUtils.isNode(cell.value) && cell.value.nodeName=='TransIm')// Apenas mostra o label para as transições imediatas
            {
                for (var i = 0; i < 1; i++)
                {
                    araux[i]=createTextField(graph, form, cell, attrs[i]);		
            ...
            function() //Função para o botão OK
            {
                ...
                while(auxbool==false)
                {
                    ...
                    else if(mxUtils.isNode(cell.value) && cell.value.nodeName=='TransIm')// Apenas modifica o label para as transições imediatas
                    {
                        (araux[0])();
                    }
                    else	
