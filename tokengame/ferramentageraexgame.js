...
function main(bodygraph, qxml, nextpage, ex, pontos)
{

    // OBSERVAÇÃO: para o arquivo ferramentageraex.js as variáveis abaixo não serão usadas. Esse arquivo visa apenas gerar XML de questões
    //bodygraph -> container do grafo
    //qxml -> xml da questão na ferramenta
    //nextpage -> próxima página após a questão
    //ex-> é o nome dado para uma determinada questão. Esse serve para indicar no localStorage se a questão foi feita ou não e a pontuação devida
    //pontos-> os pontos devido ao acerto de um determinada questão
    //Observação: os atributos de qxml seguem um formato criado pelo programador, explicado no decorrer do código
        ...
        else
        {
            ...
            //ADICIONA BOTÕES SAVE E LOAD
				...
				// TOKENGAME
				tbContainer.appendChild(mxUtils.button('GAME', function()
				{
                    var encoder = new mxCodec();
                    var node = encoder.encode(graph.getModel());			
                    var text = mxUtils.getXml(node);		
                    localStorage.setItem('tokengame', text);
                    window.open('tokengame.html','popUpWindow','height=500,width=500,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
				}));
            ...
            /////////Botão Enviar Enviar Resposta - OBS: Esse botão nesse arquivo ferramentageraex será usado para baixar o XML da questão/////////////////
    ...
    //Cria um vetor de elementos de dados dos Lugares (matriz)
    ...
    // Comparador de métricas e GuardaExpression
    ...
    // A função gerador teria mais os seguintes parâmetros qxml, nextpage, ex, pontos. Mas não será usado nesse arquivo que visa apegans gerar XML de questões
    ...
    {				
        ...
        {
            mxUtils.alert("Modelagem incorreta. Refaça novamente.");
        }*/
    ...
    function comparador(textxml, MetricaArr, TransImArr, qxml, numLugar, numTransIm, numTransTemp, numMetrica, numArcoInibidor, numArco)
    {
        ...
        if(numTransIm==l)
        {
            ...
            for(var i = 1; i <= auxl; i++)
            {
                for(var j = 1; j <= auxl; j++)
                {
                    ...
                    {						
                        if(comparadorMetrica(TransImArr, t[j-1].getAttribute('GuardaExpression'), t2[i-1].getAttribute('GuardaExpression'))==true)
                        {
                            count++;t2[i-1].remove();t[j-1].remove(); auxl--; j=0; i=0; break;
                        }
                    }
            ...
            if(count!=l){mxUtils.alert("Modelagem incorreta. Observação: as propriedades ou conexões de algumas transições imediatas estão incorretos."); return null;};
            count = 0;
        }
        ...
        if(numTransTemp==l)
        {
            ...
            if(count!=l){mxUtils.alert("Modelagem incorreta. Observação: as propriedades ou conexões de algumas transições temporizada estão incorretos."); return null;};
            count = 0;
        }
    ...
    function selectionChanged(graph, docxml)
    {
        ...
        if (cell != null)
        {
            ...
            if(mxUtils.isNode(cell.value) && cell.value.nodeName == 'TransTemp')
            {
                ...
                form.addOption(combo, 'Simgle Server','', false);
    ...
    function transtempChanged(cell)
    {
        var label = cell.getAttribute('Label','');
    ...
    function metricaAnaliselugar(strg, cell, graph, count)
    {
        ...
        for(var i = 0; i < numLugar; i++)
        {
            if(aux.indexOf(l[i].getAttribute('Label'))!=-1)
            {
                ...
                if((aux[i2]=='>')||(aux[i2]=='<')||(aux[i2]=='=')||(aux[i2]=='\/')||(aux[i2]=='}')||(aux[i2]=='\)'))
                {
                    auxcount++;
                }					
        ...
        if(aux.indexOf("#")!=-1)
        {
            ...
            {				
                ...
                else if((aux2.indexOf("1pp")!=-1))
                {
                    ...
                    if( ((aux2.match(/1p/g)).length) != count )
                    {
                        mxUtils.alert("Erro de sintaxe na métrica! '#' posicionado de maneira incorreta ou os caracteres e palavras chaves foram escritos ou posicionados de maneira incorreta. Não se calcula Esperança ou Probabilidade em GuardaExpression");
                        auxbool=false;								
                    }
                }
                else
